...
APP=$MODDIR/build/sbin/mdnsd
AVAHI_FLAGS=--file=$MODDIR/build/etc/avahi-daemon.conf
PIDNAME=avahi-daemon
...
#			pgrep $PIDNAME | xargs kill
			LD_LIBRARY_PATH=$MODDIR/build/lib $APP $CONFIG
...
#			pgrep $PIDNAME | xargs echo PID mdnsd: $1 >> $MODDIR/logging.txt
			LD_LIBRARY_PATH=$MODDIR/build/lib $MODDIR/build/bin/avahi-daemon -D $AVAHI_FLAGS && sleep 2
			LD_LIBRARY_PATH=$MODDIR/build/lib $MODDIR/build/bin/avahi-daemon -k && sleep 2
			## Make sure avahi flush cache dns if suddenly changed network
			LD_LIBRARY_PATH=$MODDIR/build/lib $MODDIR/build/bin/avahi-daemon -D $AVAHI_FLAGS
		fi
	fi
	sleep 10
done
